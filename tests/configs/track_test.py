...
def always_true_tracker(path: Path) -> bool:
    return True


def always_false_tracker(path: Path) -> bool:
    return False
...
def set_source_monkeypatch(monkeypatch, scenario: str):
    """
    scenario: one of "video", "videos_dir", "youtube"
    """
    if scenario == "video":
        monkeypatch.setitem(check_tracking_args.__globals__, "is_valid_video", always_true_video)
        monkeypatch.setitem(check_tracking_args.__globals__, "is_valid_videos_dir", always_false_videos_dir)
        monkeypatch.setitem(check_tracking_args.__globals__, "is_valid_youtube_link", always_false_youtube_link)
    elif scenario == "videos_dir":
        monkeypatch.setitem(check_tracking_args.__globals__, "is_valid_video", always_false_video)
        monkeypatch.setitem(check_tracking_args.__globals__, "is_valid_videos_dir", always_true_videos_dir)
        monkeypatch.setitem(check_tracking_args.__globals__, "is_valid_youtube_link", always_false_youtube_link)
    elif scenario == "youtube":
        monkeypatch.setitem(check_tracking_args.__globals__, "is_valid_video", always_false_video)
        monkeypatch.setitem(check_tracking_args.__globals__, "is_valid_videos_dir", always_false_videos_dir)
        monkeypatch.setitem(check_tracking_args.__globals__, "is_valid_youtube_link", always_true_youtube_link)
...
def valid_args() -> dict:
    return {
        "task": "detect",  # valid: "detect"
        "model": "model.pt",  # valid checkpoint path
        "tracker": "tracker.yaml",  # valid tracker path
        "source": "video.mp4",  # default: simulate a valid image
...
@pytest.mark.parametrize("key, invalid_value, expected_error", [
    # 'task' must be one of ['detect']
    ("task", 123, r"'task' must be one of \['detect'\]"),
    ("task", "invalid", r"'task' must be one of \['detect'\]"),
    # 'model' validated externally
    ("model", "model.pt", "'model' must be a valid .PT checkpoint file"),
    # 'tracker' validated externally
    ("tracker", "tracker.yaml", "'tracker' must be a valid .YAML tracker config file"),
    # 'source' must be one of the accepted types
    ("source", "invalid_source",
     "'source' must be a video, a directory of videos, or a youtube link"),
...
def test_invalid_args(valid_args, key, invalid_value, expected_error, monkeypatch):
    ...
    if key == "model":
        ...
        monkeypatch.setitem(check_tracking_args.__globals__, "is_valid_checkpoint", always_false_checkpoint)
    else:
        # Force model to be valid.
        monkeypatch.setitem(check_tracking_args.__globals__, "is_valid_checkpoint", always_true_checkpoint)

    if key == "tracker":
        # Force tracker to be invalid.
        monkeypatch.setitem(check_tracking_args.__globals__, "is_valid_tracker", always_false_tracker)
    else:
        # Force tracker to be valid.
        monkeypatch.setitem(check_tracking_args.__globals__, "is_valid_tracker", always_true_tracker)
    ...
    with pytest.raises(AssertionError, match=expected_error):
        check_tracking_args(args)


# -----------------------------------------------------------------------------
# Test that the function passes for valid input.
# assumes valid model, tracker and valid source (video)
# -----------------------------------------------------------------------------

def test_valid_args(valid_args, monkeypatch):
    monkeypatch.setitem(check_tracking_args.__globals__, "is_valid_checkpoint", always_true_checkpoint)
    monkeypatch.setitem(check_tracking_args.__globals__, "is_valid_tracker", always_true_tracker)
    # For default valid source, simulate valid video input.
    monkeypatch.setitem(check_tracking_args.__globals__, "is_valid_video", always_true_video)
    monkeypatch.setitem(check_tracking_args.__globals__, "is_valid_videos_dir", always_false_videos_dir)
    monkeypatch.setitem(check_tracking_args.__globals__, "is_valid_youtube_link", always_false_youtube_link)

    args = deepcopy(valid_args)
    result = check_tracking_args(args)
    ...
    # For a valid image source, "stream" should always be True.
...
# assumes valid model and tracker
# -----------------------------------------------------------------------------
