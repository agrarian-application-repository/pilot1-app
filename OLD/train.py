...
import wandb
from src.logging.wandb import get_wandb_api_key
...
def parse_args():
    ...
    parser.add_argument(
        "--data", type=str, required=True, help="Path to the dataset YAML file"
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    parser.add_argument(
        "--optimizer", type=str, default="auto", help="optimizer (default=auto)"
    )
    parser.add_argument(
        "--lr", type=float, default=1e-3, help="initial learning rate, (default=1e-3)"
    )
    parser.add_argument(
        "--device", type=str, default="3", help="device to use for training (default 3)"
    )
    parser.add_argument(
        "--workers",
        type=int,
    ...
    parser.add_argument(
        "--verbose", type=int, default=0, help="Verbose training (default: 0)"
    )
...
def main():
    ...
    # Train the model
    # TODO extend with data augmentation
    results = model.train(
