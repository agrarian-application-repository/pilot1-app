...
def perform_in_danger_analysis(
    ...
    mask_video_path="in_danger_masks.avi",
) -> None:
    # Load YOLO models
    ...
    while cap.isOpened():
        ...
        detections = (
            detection_results[0].boxes.cpu().numpy()
        )  # Assuming results are in a .boxes format
        dangerous_areas = (
        ...
        for box in detections:
            ...
            cv2.rectangle(
                mask_frame, (x, y), (x + w, y + h), (255, 0, 0), -1
            )  # Filled blue on mask frame
            ...
            cv2.ellipse(
                mask_frame, center_coordinates, axes_length, 0, 0, 360, (0, 255, 0), -1
            )  # Filled green in mask
            ...
            cv2.ellipse(
                safety_mask, center_coordinates, axes_length, 0, 0, 360, 255, -1
            )
        ...
        if np.any(intersection > 0):  # Non-zero intersection indicates overlap
            ...
            mask_frame[np.where(intersection > 0)] = (
                0,
                255,
                255,
        ...
        annotated_frame = cv2.addWeighted(
            annotated_frame, 1, dangerous_area_overlay, 0.5, 0
        )

        # Dangerous areas in red on mask frame
        mask_frame[np.where(dangerous_mask > 0)] = (
            0,
            0,
            255,
